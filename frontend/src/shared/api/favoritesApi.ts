...
export interface FavoritesDetailsResponse {
  courses: Course[];
  tasks: Task[];
}
...
export const favoritesApi = {
  ...
  getFavoritesDetails: async (): Promise<FavoritesDetailsResponse> => {
    const response = await axiosInstance.get('/favorites?detailed=true');
    return response.data;
  },
